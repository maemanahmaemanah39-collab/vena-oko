...
// Removed old CSV function - now using centralized csvUtils
...
const Clients: React.FC<ClientsProps> = ({ clients, setClients, projects, setProjects, packages, addOns, transactions, setTransactions, userProfile, showNotification, initialAction, setInitialAction, cards, setCards, pockets, setPockets, contracts, handleNavigation, clientFeedback, promoCodes, setPromoCodes, onSignInvoice, onSignTransaction, addNotification }) => {
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            ...
            try {
                ...
                const updatedProject = await SupabaseService.updateProject(selectedProject.id, updatedProjectData);
    ...
    const handleDownloadClients = () => {
        const headers = [
            'No', 'Nama Klien', 'Email', 'Telepon', 'WhatsApp', 'Instagram', 
            'Tipe Klien', 'Status', 'Total Nilai Proyek', 'Sisa Tagihan', 
            'Paket Terbaru', 'Tanggal Terdaftar', 'Lokasi Proyek Terakhir'
        ];
        
        const data = filteredClientData.map((client, index) => [
            index + 1,
            client.name,
            client.email || '',
            client.phone || '',
            ...
            client.clientType || '',
            client.status || '',
            client.totalProyekValue || 0,
            client.balanceDue || 0,
            client.paketTerbaru || '',
            client.since ? new Date(client.since).toLocaleDateString('id-ID') : '',
            client.mostRecentProject?.location || ''
        ]);
        
        import('../lib/csvUtils').then(({ downloadCSV }) => {
            downloadCSV({
                headers,
                data,
                filename: 'data-klien',
