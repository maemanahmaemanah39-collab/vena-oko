...
const Settings: React.FC<SettingsProps> = ({ profile, setProfile, transactions, projects, packages, users, setUsers, currentUser, showNotification }) => {
        ...
        if (userModalMode === 'add') {
            ...
            try {
                const newUser = await SupabaseService.createUser({
                    ...
                    role: userForm.role,
                    permissions: userForm.role === 'Member' ? userForm.permissions : Object.values(ViewType),
                    isApproved: true
                });
                setUsers(prev => [...prev, newUser]);
                showNotification('Pengguna berhasil dibuat!');
            } catch (error: any) {
                console.error('Error creating user:', error);
                setUserFormError('Gagal membuat pengguna: ' + error.message);
                return;
            }
            ...
            try {
                ...
                const updatedUser = await SupabaseService.updateUser(selectedUser.id, updates);
                setUsers(prev => prev.map(u => u.id === selectedUser.id ? updatedUser : u));
                showNotification('Pengguna berhasil diperbarui!');
            } catch (error: any) {
                console.error('Error updating user:', error);
                setUserFormError('Gagal memperbarui pengguna: ' + error.message);
                return;
            }
        }
    ...
    const handleApproveUser = async (userId: string) => {
        try {
            const updatedUser = await SupabaseService.updateUser(userId, { isApproved: true });
            setUsers(prev => prev.map(u => u.id === userId ? updatedUser : u));
            showNotification('Pengguna berhasil disetujui dan sekarang dapat login.');
            handleCloseUserModal();
        } catch (error: any) {
            console.error('Error approving user:', error);
            alert('Gagal menyetujui pengguna: ' + error.message);
        }
    };
