    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (modalMode === 'add') {
                ...
                const updatedContract = await SupabaseService.updateContract(selectedContract.id, updatedContractData);
            ...
            showNotification('Gagal menyimpan kontrak. Silakan coba lagi.');
        }
        handleCloseModal();
    };
    ...
    const handleDelete = async (contractId: string) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus kontrak ini?")) {
            try {
                ...
                showNotification('Gagal menghapus kontrak. Silakan coba lagi.');
            }
        }
    };
