...
export const SocialPlanner: React.FC<SocialPlannerProps> = ({ posts, setPosts, projects, showNotification, profile }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (modalMode === 'add') {
                ...
                const updatedPost = await SupabaseService.updateSocialMediaPost(selectedPost.id, postData);
                setPosts(prev => prev.map(p => p.id === selectedPost.id ? updatedPost : p));
                showNotification('Postingan berhasil diperbarui.');
            }
        } catch (error) {
            console.error('Error saving post:', error);
            showNotification('Gagal menyimpan postingan. Silakan coba lagi.');
        }
        handleCloseModal();
    };
    ...
    const handleDelete = async (postId: string) => {
        if(window.confirm('Yakin ingin menghapus postingan ini?')) {
            try {
                await SupabaseService.deleteSocialMediaPost(postId);
                setPosts(prev => prev.filter(p => p.id !== postId));
                showNotification('Postingan berhasil dihapus.');
                handleCloseModal();
            } catch (error) {
                console.error('Error deleting post:', error);
                showNotification('Gagal menghapus postingan. Silakan coba lagi.');
            }
        }
    }
