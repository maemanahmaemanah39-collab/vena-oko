...
export const CalendarView: React.FC<CalendarViewProps> = ({ projects, setProjects, teamMembers, profile }) => {
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (selectedEvent) { // Editing
                ...
                const updatedProject = await SupabaseService.updateProject(selectedEvent.id, updatedEventData);
                setProjects(prev => prev.map(p => p.id === selectedEvent.id ? updatedProject : p));
            ...
            // Handle error notification if showNotification is available
        }
    };
    ...
    const handleDeleteEvent = async () => {
        if (selectedEvent && window.confirm(`Yakin ingin menghapus acara "${selectedEvent.projectName}"?`)) {
            try {
                await SupabaseService.deleteProject(selectedEvent.id);
